...
| 表名 | 职责 | 分片键 |
|------|------|--------|
| `component_comment` | 一级评论主表 | `comment_object_id` |
| `component_comment_reply` | 回复 / 楼中楼表 | `comment_id` |
| `component_comment_like` | 点赞记录表 | — |
| `component_comment_audit` | 审核记录表 | — |
| `component_user_comment_index` | 用户评论索引表 | `user_id` |

> 建表脚本见 [`docs/ddl.sql`](docs/ddl.sql)
...
- 评论表按 `comment_object_id` 分片 → 对象详情页评论列表命中单分片
- 回复表按 `comment_id` 分片 → 评论回复列表命中单分片
- 通过 `CommentShardRouter` 在源码层体现路由思路，真实路由由 ShardingSphere 接管
...
| [`docs/ddl.sql`](docs/ddl.sql) | 数据库建表脚本 |
