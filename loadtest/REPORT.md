...
2. **热评缓存与库不一致（发现一个 BUG）**：Redis 里 `comment:hot:obj:10001:type:1` zset 若残留**旧 ID**，`queryByIds` 会因 `where id in (...) and comment_object_id=10001` 匹配不到而返回空列表，导致 hot 接口「有数据却返回空」。清掉 zset 让浏览器回源 DB 后即恢复正常。→ 需要缓存一致性/失效策略（如 TTL、事件驱动重建、启动时预热）。
   ✅ **已修复（2026-09-14）**：热评榜与点赞数缓存各加 **1 小时 TTL**（脏数据最多留 1 小时自动消失）；读路径加 try/catch **降级回源 DB**；热度公式由 `likeCount*100 + 当前秒时间戳`（实际退化成"最新优先"）改为 `likeCount*1000000 + 创建时间(小时)`；点赞计数改原子自增。改动详情见 `docs/01-design/待办-缓存与热点加固.md` 的「实施记录」。**注意：本文以下所有压测数字仍是加固前跑出来的基线**，加固后需按 §6 重跑对比。
