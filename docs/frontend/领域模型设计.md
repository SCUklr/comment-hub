...
> 本文档面向前端展示页开发，梳理评论中心核心领域实体、关系及数据约束。模型基于现有后端代码（`ssp-comment-center-domain`）与数据库 DDL（`docs/database/ddl.sql`）整理。
...
- 创建评论/回复后，通过 Spring Event 异步更新索引表，允许秒级延迟。
- 删除评论/回复时，同步或异步将对应索引标记为 `is_delete = 1`。

---

## 四、面向前端展示页的 VO 映射

前端展示页主要消费以下后端返回结构（已存在于 `ssp-comment-center-start/vo/resp`）：
...
前端不需要直接感知数据库 PO/Entity，只需按 VO 结构渲染。
