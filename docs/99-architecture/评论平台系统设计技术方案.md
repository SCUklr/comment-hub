...
> ⚠️ **来源与口径提示**：本文档是项目立项期的**课程蓝图（设计态）**。其中 RabbitMQ / Elasticsearch / 定时任务等设计**均未实现**，异步方案最终落地为 Spring Event；已实现 / 未实现清单以 `docs/02-interview/面试总纲.md` §0.1 为准。
>
> 📦 文中原有配图引用作者本机绝对路径（`~/comment-project/`、`~/Downloads/`、Typora 图床），未随仓库分发，已统一替换为「图缺失」占位说明，避免克隆后出现断图。
...
graph TD
    ...
    J --> I
```
> 🖼 **图缺失**：Untitled Diagram.drawio（原配图为本机绝对路径，未随仓库分发）

##### 2.2.2.1.1 时序图

> 🖼 **图缺失**：mermaid-diagram（原配图为本机绝对路径，未随仓库分发）
...
> 🖼 **图缺失**：t2-delete-comment1.drawio（原配图为本机绝对路径，未随仓库分发）



> 注：原流程图需调整，移除 “级联删除回复”“扣减回复数” 分支，仅保留 “删评论→清理缓存→返回结果” 核心链路。

> 🖼 **图缺失**：image-20260311154358206（原配图为本机绝对路径，未随仓库分发）
...
> 🖼 **图缺失**：2.1.8（原配图为本机绝对路径，未随仓库分发）

##### 2.2.2.8.1 时序图

> 🖼 **图缺失**：mermaid-diagram (1)（原配图为本机绝对路径，未随仓库分发）
...
> 🖼 **图缺失**：2.1.9（原配图为本机绝对路径，未随仓库分发）

#### 2.2.2.10 评论点赞流程

> 🖼 **图缺失**：image-20260311160625352（原配图为本机绝对路径，未随仓库分发）

#### 2.2.2.11 内容审核流程

> 🖼 **图缺失**：image-20260311160724163（原配图为本机绝对路径，未随仓库分发）
...
> 🖼 **图缺失**：mermaid-diagram (2)（原配图为本机绝对路径，未随仓库分发）
...
> 🖼 **图缺失**：exported_image (3)（原配图为本机绝对路径，未随仓库分发）

### 2.2.4 类图设计

> 🖼 **图缺失**：image-20260311161122687（原配图为本机绝对路径，未随仓库分发）
