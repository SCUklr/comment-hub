...
> 本文档面向上游业务方接入与系统讲解，梳理评论中心核心领域实体、关系及数据约束。模型基于现有后端代码（`ssp-comment-center-domain`）与**分库分表建表脚本**（`docs/05-database/ddl-sharding.sql`）整理。
>
> ⚠️ `docs/05-database/ddl.sql` 是**早期单库版本**（库名 `ssp_comment`，无分片表），仅作历史参考，**不要用它建库**。
