# 接口测试指南（curl + Postman + 常见问题）

> 本文档记录 ssp-comment-center 评论中心全部 API 接口的测试方法，包含 curl 命令、Postman/Apifox 配置步骤、环境变量与断言脚本、完整测试流程、异步监听器验证、站内通知与审核状态机测试、字段说明及踩坑记录。  
> 适合豆包、ChatGPT、Cursor、Claude 等 LLM 快速阅读与检索。
>
> 本文件由 `测试用例以及如何测试.md` 与 `API测试指南.md` 合并而成（2026-09-14）：前者已重命名为本文件，后者已删除（原文件可在 git 历史中查看）。
...
| 通知 | 我的通知列表 | `GET` | `/api/notification/list` | ✅ |
| 通知 | 标记单条已读 | `POST` | `/api/notification/read` | ✅ |
| 通知 | 标记全部已读 | `POST` | `/api/notification/read/all` | ✅ |
| 审核 | 审核历史 | `GET` | `/api/comment/audit/history` | ❌ |
...
```
1. 创建评论（用户 1）→ 提取 commentId，默认 audit_status=1（未登录用户可见）
2. 回复评论（用户 2）→ 提取 replyId，默认 audit_status=1
...
12. 审核回调（auditStatus=2）→ 验证非作者看不到该内容，作者仍可见，热评缓存与用户索引被清理
13. 删除评论（用户 1）→ 验证列表为空
```
...
> 文档生成时间：2026-06-11，合并整理：2026-09-14  
